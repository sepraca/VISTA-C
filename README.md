...
*(The hosted version tracks `main`, which is currently at the tagged **v6.5.1** release
...
Latest tagged release: **v6.5.1** (2026-08-13, iOS PNG export fix — no physics change; both goldens bit-identical, and both desktop PNG exports — bottom panel and 3-D view — pixel-identical to v6.5.0). Both PNG exports built a `data:` URL and handed it to an `<a download>` click, which iOS Safari ignores entirely: no file, no error, no exception. All exports now share one blob-based transport (`canvas.toBlob` → `createObjectURL`), the same mechanism the JSON export already used and which worked on iOS throughout. Adds `verify_export_download.mjs` (13 suites) to guard the transport, since the defect was invisible to every runtime gate. See CHANGELOG.md's `[v6.5.1]` section.

Previous release: **v6.5.0** (2026-08-12, phone and small-screen layout — display only, no physics change; both goldens bit-identical, and both desktop PNG exports — bottom panel and 3-D view — byte-identical to v6.4.0 by SHA-256). Adds the viewport meta tag that mobile browsers need, a bottom panel bounded on both axes instead of fixed at 700 px, and an overlay-drawer layout below 930 px (`BASE_W × MIN_SCALE`, derived rather than hand-picked) where the control panel would otherwise take 64 % of a phone screen. Desktop and tablet rendering are unchanged. See CHANGELOG.md's `[v6.5.0]` section.
...
v6.5.1 is the version currently on `main` and in the hosted demo.
...
> Platnick, S. (2026). *VISTA-C: An Interactive 3D Monte Carlo Visualization of Cloud Radiative Transfer* (v6.5.1). GitHub. https://github.com/sepraca/VISTA-C